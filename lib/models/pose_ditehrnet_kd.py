...
class GlobalContextModeling(nn.Module):
    ...
    def global_spatial_pool(self, x):
        ...
        x_m = x
        # [N, C, H * W]
        #print("zh1",x_m.shape)#zh1 torch.Size([1, 16, 64, 64])
        x_m = x_m.view(batch, channel, height * width)
        # [N, 1, C, H * W]
        #print("zh2",x_m.shape)#zh2 torch.Size([1, 16, 4096])
        x_m = x_m.unsqueeze(1)
        #print("zh3",x_m.shape)#zh3 torch.Size([1, 1, 16, 4096])
        # [N, 1, H, W]
        mask = self.conv_mask(x)
        #print("zh4",mask.shape)#zh4 torch.Size([1, 1, 64, 64])
        # [N, 1, H * W]
        mask = mask.view(batch, 1, height * width)
        #print("zh5",mask.shape)#zh5 torch.Size([1, 1, 4096])
        # [N, 1, H * W]
        mask = self.softmax(mask)
        #print("zh6",mask.shape)#zh6 torch.Size([1, 1, 4096])
        # [N, 1, H * W, 1]
        mask = mask.unsqueeze(-1)
        #print("zh7",mask.shape)#zh7 torch.Size([1, 1, 4096, 1])
        # [N, 1, C, 1]
        x = torch.matmul(x_m, mask)
        #print("zh8",x.shape)#zh8 torch.Size([1, 1, 16, 1])
        # [N, C, 1, 1]
        x = x.permute(0, 2, 1, 3)
        #print("zh9",x.shape)#zh9 torch.Size([1, 16, 1, 1])
        return x
...
class DiteHRNet(nn.Module):
    ...
    def init_weights(self, pretrained=''):
        ...
        if os.path.isfile(pretrained):
            ...
            logger.info('=>teacher loading pretrained model {}'.format(pretrained))

            self.load_state_dict(pretrained_state_dict, strict=False)
        elif pretrained:
            logger.error('=> teacher please download pre-trained models first!')
    ...
    def forward(self, x):
        ...
        if self.with_head:
            x = self.head_layer(x)
        out= self.final_layer(x[0])
        #print(len(c[0]),len(c[1]))
        
        #print(len(c[0]),len(c[1]))
        return c[0],c[1],c[2],out
        ...
        if mode and self.norm_eval:
            for m in self.modules():
                if isinstance(m, (_BatchNorm, nn.BatchNorm2d)):
                    m.eval()
def get_pose_net_kd(cfg, is_train, **kwargs):
    model = DiteHRNet(cfg, **kwargs)
    if is_train and cfg['MODEL']['INIT_WEIGHTS']:
        model.init_weights(cfg['MODEL']['TEACHER'])
